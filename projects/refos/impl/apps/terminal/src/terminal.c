...
int main(void)
{
    ...
    for(int i = 0; i < 100; i++)
    {
      seL4_DebugPrintf("array[%d] is : %lld\n", i, array[i]);
    }
    ...
    int a = 100;
    //error = data_write(REFOS_PROCSERV_EP, dspaceCap, 0, &a, 4);
    ...
    *start = a;
    ...
    //error = data_read(REFOS_PROCSERV_EP, dspaceCap, 0, &b, 4);

    seL4_DebugPrintf("\n!!!, the number write to dspace is %d", a);

...
int main(void)
{
    ...
    seL4_CPtr userB_cptr = proc_get_hello3_cptr();
    seL4_CPtr userC_cptr = proc_get_hello4_cptr();
    ...
    while(1)
    {
        ...
        if (i % 2 == 0)
        {
            seL4_TCB_Resume(userB_cptr);
        }
        else
        {
            seL4_TCB_Resume(userC_cptr);
        }
        //print_cycle("local B finishes!");

        //cycle = get_cycle();
        //*(start+2) = cycle;
        //seL4_DebugPrintf("in localB, cycle 1 is %lld\n", *start);
        //seL4_DebugPrintf("in localB, cycle 3 is %lld\n", cycle);
        //seL4_DebugPrintf("from A to B, overhead is %lld\n", (cycle - *start)/2400);

        cycle2 = get_cycle();
        seL4_DebugPrintf("Local A 's overhead is %lld us\n", (cycle2 - cycle1)/2400);

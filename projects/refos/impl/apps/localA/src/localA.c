...
int main(void)
{
    ...
    seL4_CPtr userA_cptr = proc_get_hello2_cptr();
    ...
    while(1)
    {
        ...
        seL4_TCB_Resume(userA_cptr);
        //print_cycle("local A finishes!");
        
        cycle = get_cycle();
        //*(start+1) = cycle;
        //seL4_DebugPrintf("in localA, cycle 1 is %lld\n", *start);
        //seL4_DebugPrintf("in localA, cycle 2 is %lld\n", cycle);
        //seL4_DebugPrintf("from B to A, overhead is %lld\n", (cycle - *start)/2400);
